...
| C31 | P3 | Awaria transportu przy POTWIERDZAJACYM odczycie po delete "object missing" niesie obie przyczyny (pierwotna odpowiedz + blad odczytu) zamiast maskowac jedna druga; helper acceptance honoruje `ZABBIX_TLS_INSECURE`/`ZABBIX_CA_CERT_FILE`; `localhost` porownywany bez uwzglednienia wielkosci liter (Codex r35) | `TestDeleteError_ConfirmFailureSurfacesBoth`, `TestHelperClientTLSEnv`, wariant LOCALHOST w testach loopback | DONE |
...
| R7 | P2 | Granica zaufania release: ruleset `protect-release-tags` (tagi `v*` - creation/update/deletion/non-FF tylko dla admina repo) uniemozliwia osobie z prawem zapisu wypchniecie taga ze zmodyfikowanym workflow poza kontrola ancestry; rezydualne ryzyko = kompromitacja konta admina (rownowazna kontroli nad main), opcjonalny required reviewer na env `release` pozostaje po stronie wlasciciela (Codex r39) | ruleset 21924614 w repo SychPL | DONE |
...
| T26 | P3 | Cleanup tokenu w tescie akceptacyjnym: swiezy deadline (30s) i blad zgloszony przez `t.Errorf` - awaria transportu nie zostawia po cichu aktywnego tokenu na docelowym Zabbixie (Codex r39) | `TestAccProvider_APIToken` | DONE |
| T25 | P3 | Macierz acceptance obejmuje takze 6.4.1 - MINIMALNA deklarowana wersje (overlay `docker-compose.acc-641.yml`, digest-pinned; `ZABBIX_ACC_EXPECT_VERSION=6.4.1`) - regresja w zakresie 6.4.1-6.4.20 nie wyjdzie dopiero u uzytkownika (Codex r36) | job "6.4.1" w ci.yml | DONE |
...
| H20 | P2 | Niekompletny wpis interfejsu w odpowiedzi `host.get` (brak `interfaceid`/`type`/`main`) = odmowa w Read i preflight update - czesciowa odpowiedz nie udaje "braku interfejsu agenta" (apply tworzylby duplikat glownego interfejsu) (Codex r38) | `TestHostRead_RefusesIncompleteInterface` | DONE |
| H19 | P2 | Read i preflight update hosta fail-closed na wartosciach niereprezentowalnych (wspolny `unmanageableHost`: status/useip poza {0,1}, adresy interfejsu przez walidatory schematu) - odpowiedz posrednika nie znormalizuje sie cicho do false/defaultow (Codex r37) | `TestHostRead_RefusesUnrepresentableValues` | DONE |
...
| M20 | P2 | Read i preflight update media typu fail-closed na ZAKRESACH wlasnych pol (wspolny `mediaTypeInts`: enumy, porty, max_attempts, attempt_interval, timeout webhooka) - wartosc z nowszej linii API nie zatruje stanu; walidacja resolved w Create/Update powtarza kazdy walidator schematu (Codex r35) | `TestMediaTypeRead_RefusesOutOfRangeValues` | DONE |
...
| A27 | P2 | Wymagane bity binarne akcji musza byc OBECNE: puste `status`/flagi pauzy = odmowa zamiast mapowania na false (czesciowy `action.get` ukrylby drift: konfiguracja false + rzeczywiste true = pusty plan) (Codex r38) | przypadki "missing" w `TestActionRead_RefusesOutOfModelValues` | DONE |
| A26 | P2 | `flattenAction` odmawia `status`/`pause_suppressed`/`pause_symptoms`/`notify_if_canceled` poza {0,1} zamiast normalizowac do false (kolejny update wyslalby status=1/flag=0 do Zabbixa); docs opisuja pierwszenstwo jawnej konfiguracji nad srodowiskowa zamiast ogolnego "mutually exclusive" (Codex r37) | przypadki binarne w `TestActionRead_RefusesOutOfModelValues`, `templates/index.md.tmpl` | DONE |
| A25 | P2 | `flattenAction` fail-closed na PELNYM modelu: evaltype poza {0,1,2}, default_msg poza {0,1}, kroki eskalacji, okresy i reguly wartosci warunkow (severity 0-5, time period, value2 tylko dla 26, niepusta wartosc) - nieobslugiwany ksztalt nie wchodzi do stanu (Codex r35) | `TestActionRead_RefusesOutOfModelValues` | DONE |
...
- (Codex, r39) "Bramke logow zastapic regula AST/Semgrep obejmujaca tflog": obecny kod nie
  loguje zadnych payloadow (zweryfikowane grep + recenzje 39 rund); bramka grep broni przed
  regresja najprostszej klasy, a pelny linter AST to narzedzie procesowe zakresu v0.3 -
  trigger wymaga przyszlej, hipotetycznej zmiany kodu, nie stanu obecnego (SPEC sekcja 6).
