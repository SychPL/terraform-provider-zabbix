...
Tested against **Zabbix 6.4** (see [docker-compose.yml](docker-compose.yml)).
...
```sh
docker compose up -d --wait         # Zabbix 6.4 on http://localhost:8082 (Admin / zabbix)
TF_ACC=1 ZABBIX_URL=http://localhost:8082/api_jsonrpc.php \
  ZABBIX_USERNAME=Admin ZABBIX_PASSWORD=zabbix \
  go test ./zabbix -run TestAcc -v
...
provider_installation {
  dev_overrides {
    "adi/zabbix" = "/path/to/terraform-provider-zabbix"
  }
  direct {}
}
```
...
cd test_deployment && terraform apply
```
